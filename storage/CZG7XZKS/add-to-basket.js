(function ($, AOP) {
  ...
  function initializePrintCopyModal () {
    buyPrintCopyButton.on('click', function (e) {
      ...
      var message = 'To complete your purchase of print-on-demand copies, you will be directed to Sheridan Custom Publishing. ' + 'Any items added to your basket for purchase on the Cambridge Core site will remain in your basket and can be purchased ' + 'separately from print-on-demand purchases.';
  ...
  function isProductInBasket (id) {
    return (
      basketItemIds.filter(function (item) {
        return item === id;
      }).length > 0
    );
  }
