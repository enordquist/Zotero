...
// vvvvvvvvvvvv CAN BE REMOVED AFTER TESTING - ask bm vvvvvvvvvvvvvvvvvvvvv
// This functionality now in vat.js on scripts server (bm: 20200513)
...
// // publications; GB moved out of array so it does not trigger change in rubric
// // GB:"0.000", 
// var vat_rate={SE:"0.250", LT:"0.210", ES:"0.210", RO:"0.190", LU:"0.170", GR:"0.240", LV:"0.210", EE:"0.200", SI:"0.220", PT:"0.230", CY:"0.190", IT:"0.040", CZ:"0.210", SK:"0.200", IE:"0.230", BE:"0.210", FR:"0.200", MT:"0.180", BG:"0.200", DE:"0.190", NL:"0.210", HR:"0.250", PL:"0.230", HU:"0.270", AT:"0.200", DK:"0.250", FI:"0.240"};
//
// // Latest update 2020-06-09 (bm) from email of Alex Ashcroft - reduced
// // rates for pay-per-view. Now sorted alphabetically by country name
// var vat_rate={AT:"0.100", BE:"0.060", BG:"0.200", HR:"0.050", CY:"0.050", CZ:"0.100", DK:"0.250", EE:"0.090", FI:"0.100", FR:"0.021", DE:"0.070", GR:"0.060", HU:"0.050", IE:"0.090", IT:"0.040", LV:"0.120", LT:"0.050", LU:"0.030", MT:"0.050", NL:"0.090", PL:"0.050", PT:"0.060", RO:"0.050", SK:"0.200", SI:"0.095", ES:"0.040", SE:"0.060"};
// 
...
//     }
// });
// ^^^^^^^^^^^^ CAN BE REMOVED AFTER TESTING - ask bm ^^^^^^^^^^^^^^^^^^^^^
...
var spwPageFuncs = (function() {
  ...
  function _modifyCCDClinks() {
    // Aug 2020 - hide long lists of ccdc links - see https://journals.iucr.org/m/issues/2020/05/00/index.html
    // and https://journals.iucr.org/m/issues/2020/05/00/lt5031/index.html
    
    var ccdclinks=document.getElementsByClassName("art_codelinks_csd");
    if (ccdclinks&&ccdclinks.length>0) { // could be more than one - art + sup!
         for (var cd=0;cd<ccdclinks.length;cd++) {
            var ccdcls=ccdclinks[cd].getElementsByTagName("a");
            if (ccdcls&&ccdcls.length>10) {
                // hide the first par in the ccdc div
                var fps=ccdclinks[cd].getElementsByTagName("p");
                if (fps&&fps.length>0) {
                    var p=fps[0];
                    p.style.display="none";
                    p.getElementsByClassName("art_codelinks_head")[0].style.display="none";
                    var np=document.createElement("p");
                    np.innerHTML="<span class='art_codelinks_head'><img src='//journals.iucr.org/logos/arrows/smarrr.png' /> CCDC references</span>";
                    np.refp=p;
                    np.onclick=function() {
                        if (this.refp.style.display&&this.refp.style.display=="block") 
                        {
                            this.refp.style.display="none";
                            this.getElementsByTagName("img")[0].src='//journals.iucr.org/logos/arrows/smarrr.png';
                        }
                        else {
                            this.refp.style.display="block";
                            this.getElementsByTagName("img")[0].src='//journals.iucr.org/logos/arrows/smarrd.png';
                        }
                    };
                    ccdclinks[cd].insertBefore(np, p);
     ...
     // could be on contents pages:
     var loc=window.location.href;
     var rex = /journals\.iucr\.org\/([abcdfjsm])\//g;
     var m = rex.exec(loc);
     if (!m) return;

     var ccdclinks_alt=document.getElementsByClassName("ica_codelinks_csd"); // contents pages - bit different
     var changeels=[]; 
     if (ccdclinks_alt&&ccdclinks_alt.length>0) { // could be more than one - art + sup!
         for (var cd=0;cd<ccdclinks_alt.length;cd++) {
            var ccdcls=ccdclinks_alt[cd].getElementsByTagName("a");
            if (ccdcls&&ccdcls.length>10) {
                // wrap this in a new div
                changeels.push(ccdclinks_alt[cd]);
     ...
     for (var x=0;x<changeels.length; x++) {
         
                changeels[x].getElementsByClassName("ica_codelinks_head")[0].style.display="none";
                changeels[x].style.display="none";
                changeels[x].style.paddingTop=".5em";
                var oh=changeels[x].innerHTML;
                changeels[x].innerHTML=oh.replace(/^\s*<span[^>]+>.*?<\/span>\s*:\s*/, ""); // annoying colon outside span!
                var ndiv=document.createElement("div");
                changeels[x].parentNode.insertBefore(ndiv, changeels[x]);
                var np=document.createElement("div");
                np.innerHTML="<span class='ica_codelinks_head'><img src='//journals.iucr.org/logos/arrows/smarrr.png' /> CCDC references</span>";
                np.onclick=function() {
                    if (this.nextSibling.style.display&&this.nextSibling.style.display=="block") 
                    {
                ...
                ndiv.appendChild(changeels[x]);
                ndiv.className="ica_codelinks_csd";
...
function addTrendMD() {
    // modified Oct 2020 to be jnl specific:
    var _jnl="";
    if (document.getElementById("jpage_c")) {
        _jnl="c";
    }
 ...
 // var content = element && element.getAttribute("content");
  ...
  if (iucrart) {
    ...
    trendmdscript.setAttribute('data-trendmdconfig','{"journal_id":"'+trendMDsite[_jnl]+'","element":"#trendmd-suggestions"}'); // oct 2020)
...
else if (window.attachEvent) window.attachEvent('onload', function() {addTrendMD();});
