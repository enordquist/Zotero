...
//window['ga-disable-UA-187984-1'] = true;
...
function sup_cif_init() {
  ...
  // June 2016 - get coedid from meta info first and add PPT link
    ...
    // Video
        ...
        var supmdiv;
        var supmata;
        if (_getCookie("iucr_cookie_prefs")!="1") { // Nov 2020 - users can opt out of third-party stuff
...
//ORIGINAL CIF SUP STUFF
  ...
  var popup=false;
    ...
    function _setCookie(cname, cvalue, exdays) {
        ...
        document.cookie = cname + "=" + cvalue + ";" + expires + ";path=/; domain=iucr.org; samesite=lax; secure";
    }
    ...
    function _deleteCookies(pattern) {
        if (!pattern) {
            pattern=/^\s*(__at|__ut)/i;
        }
        var remove=[];
        var ca = document.cookie.split(';');
        for(var i = 0; i < ca.length; i++) {
        ...
        for (var r=0;r<remove.length; r++) {
            _removeCookie(remove[r]);
        }
        
    }
    function _removeCookie(cname) {
        console.log(cname + "=0;expires=Thu, 01 Jan 1970 00:00:01 GMT;path=/");
